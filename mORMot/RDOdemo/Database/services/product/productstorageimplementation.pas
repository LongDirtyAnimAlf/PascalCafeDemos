...
type
  TProductStorage = class(TRunDataStorage, IProductStorage)
  strict private
  ...
  private
    ...
    procedure GetTestData(var AProduct: TProduct);
...
begin
  ...
  begin
    ...
    // Create the server connected to the SQLite database for Battery and RunData
    fRestServerDB := TRestServerDB.Create(TOrmModel.Create([TOrmProduct, TOrmRunData]), aDBFile);
    ...
    fRestServerDB.CreateSqlMultiIndex(TOrmRunData,['ProductCode','DischargeType','DischargeValue','DataInvalid'],false);
  ...
  inherited Create(fRestOrm);

  TOrmProduct.AddFilterNotVoidText(['ProjectNumber']);
  ...
  fMap.Map([
    ...
    'Brand'            ,'B_name',
    'Model'            ,'B_type',
    'Capacity'         ,'StatedCapacity',
    'ProjectNumber'    ,'Test_id'
    ]);
end;
...
begin
  ...
  // Also get the testdata for this battery !!
  GetTestData(AProduct);
end;

procedure TProductStorage.GetTestData(var AProduct: TProduct);
var
  OrmRunData:TOrmRunData;
  where: RawUtf8;
begin
  // This is a bit of a hack
  // As the testdata values are stored into the orm rundata, we need to extract them to store the testdata into the battery DTO.
  // This is due to legacy databases, but we keep it like this

  AProduct.TestDatas.Clear;

  where := FormatSql(MAINFIELD+' = ?', [], [AProduct.Code]);
  OrmRunData:=TOrmRunData.CreateAndFillPrepare(fRestOrm,where,'Distinct DischargeType,DischargeValue');
  try
    while OrmRunData.FillOne do
    begin
      with AProduct.TestDatas.Add do
      begin
        StageMode:=OrmRunData.DischargeType;
        SetValue:=OrmRunData.DischargeValue;
      end;
    end;
  finally
    OrmRunData.Free;
