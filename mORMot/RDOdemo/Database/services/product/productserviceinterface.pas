...
type
  IProductService = interface(IRunDataService)
  ...
  function RunDataFieldsToVariant(const ARunData: TRunData; const Fieldinfo:RawUTF8; out TD:variant):boolean;
        ...
        begin
          ...
          begin
            ...
            begin
              if (pa^.Value.ValueRtlClass=vcCollection) then
              begin
                // Skip the data collections
                if NOT SkipField then SkipField:=(pa^.Value.Info^.RttiClass^.RttiClass.ClassParent=TTestCollection);
...
begin
  ...
  //rA := Rtti.RegisterClass(TRunData);
  rA := Rtti.RegisterClass(PClass(ARunData)^);
      ...
      repeat
        begin
          ...
          begin
            ...
            begin
              // Get fielddata as variant
              pa^.GetValueVariant(ARunData,TVarData(TD2), @JSON_[mDefault]);
              // Add requested fieldname and fielddata
              TDocVariantData(TD).AddValue(pa^.Name,TD2);
            end;
          end;
